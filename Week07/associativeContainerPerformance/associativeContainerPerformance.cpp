...
int main()
{
    ...
    std::chrono::duration<double> dur = std::chrono::system_color::now() - start;
    std::cout << "time for std::map:" << dur.count() << "seconds" << std::endl;

    auto start2 = std::chrono::system_color::now();
    for (long long i = 0; i < accSize; ++i) {
        myHash[randValues[i]];
    }
    std::chrono::duration<double> dur2 = std::chrono::system_color::now() - start2;
